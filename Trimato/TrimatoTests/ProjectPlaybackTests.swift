...
struct ProjectPlaybackTests {
    ...
    @Test func timelineKeysRespectMovementModeAndVoiceOverModifiers() {
        ...
        #expect(TimelineKeyAction.resolve(keyCode: 51, modifiers: [], isMoving: false) == .delete)
        #expect(TimelineKeyAction.resolve(keyCode: 117, modifiers: [], isMoving: false) == .delete)
        #expect(TimelineKeyAction.resolve(keyCode: 51, modifiers: [.command], isMoving: false) == nil)
    }
    ...
    @Test @MainActor func crossDissolvePreviewRendersSourcesWithDifferentFrameRates() async throws {
        ...
        var renderedAudioDurations: [Double] = []
        for url in result.temporaryMediaURLs {
            ...
            if report.videoStream == nil, report.hasAudio {
                renderedAudioDurations.append(report.duration)
            }
        }
        // Source-format intermediates can precede the transition render.
        #expect(renderedAudioDurations.contains { abs($0 - duration.seconds) < 0.02 })
        let audioTracks = try await result.composition.loadTracks(withMediaType: .audio)
