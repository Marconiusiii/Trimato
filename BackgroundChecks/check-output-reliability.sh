...
check_root="${0:A:h}"
scratch="$(mktemp -d "${TMPDIR:-/tmp}/trimato-output-reliability-check.XXXXXX")"
...
xcrun swiftc -parse-as-library -module-cache-path "$scratch/ModuleCache" \
    -I "$products" "$check_root/OutputReliabilityCheck.swift" \
    "$binary_directory/Trimato.debug.dylib" \
    -Xlinker -rpath -Xlinker "$binary_directory" -o "$scratch/Check.app/Contents/MacOS/check"
"$scratch/Check.app/Contents/MacOS/check" "$check_root/../appStore/test_videos/4K_30fps.MOV"
